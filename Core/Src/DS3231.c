...
void time_write(int hr,int min,int sec,int format)
{
	...
	HAL_I2C_Mem_Write(&hi2c1, Devaddress, MemAddress, 1, (uint8_t*) &pata,3,1000);
...
void date_write(int day,int date,int month,int year)
{
	...
	HAL_I2C_Mem_Write(&hi2c1, Devaddress, MemAddress, 1,(uint8_t *) &pata,4,1000);
...
int read_time( int type)
{
	...
	uint16_t Devaddress=(Dev<<1);
	 // HAL_I2C_Master_Transmit(&hi2c1,Devaddress, pData,2,1000);
	  ...
	  uint8_t data[5];
	  HAL_I2C_Mem_Read(&hi2c1, Devaddress,MemAddress,1,(uint8_t *)&data,3,1000);
...
switch (type) {
	...
	case 2: return minute; break;
	case 3: return hour; break;
}	
}

int read_date(int type)
{
	uint8_t MemAddress=DATE;

	uint16_t Devaddress=(Dev<<1);
	  //HAL_I2C_Master_Transmit(&hi2c1,Devaddress, (uint8_t *)&pata,2,1000);

	  uint8_t data[5];
	  HAL_I2C_Mem_Read(&hi2c1, Devaddress,MemAddress,1,(uint8_t *)&data,4,1000);
uint8_t day=data[0];
uint8_t date=data[1];
